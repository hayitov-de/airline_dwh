...
create_raw_table = """
    drop table if exists raw.seats;
    
    create table raw.seats (
...
"""
insert_sql = """
    insert into raw.seats (
        airplane_code, seat_no, fare_conditions, _loaded_at, _source_system
    ) values %s
"""

insert_template = "(%s, %s, %s, %s, %s)"
...
def main():
    ...
    try:
        ...
        print(f"Загружено в raw.seats: {len(rows)} строк, _loaded_at={load_ts.isoformat()}")
    finally:
        source_conn.close()
        dwh_conn.close()
