...
create_raw_table = """
    drop table if exists raw.airplanes_data;
    
    create table raw.airplanes_data (
...
"""
insert_sql = """
    insert into raw.airplanes_data (
        airplane_code, model, range, speed, _loaded_at, _source_system
    ) values %s
"""

insert_template = "(%s, %s::jsonb, %s::int, %s::int, %s, %s)"
...
def main():
    ...
    try:
        ...
        print(f"Загружено в raw.airplanes_data: {len(rows)} строк, _loaded_at={load_ts.isoformat()}")
    finally:
        source_conn.close()
        dwh_conn.close()
