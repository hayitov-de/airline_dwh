...
extract_sql = """
    select 
        book_ref,
        book_date::text as book_date,
        total_amount::text as total_amount
    from bookings.bookings
"""
insert_sql = """
    insert into raw.bookings (
        book_ref, book_date, total_amount, _loaded_at, _source_system
    ) values %s
"""

insert_template = "(%s, %s::timestamptz, %s::numeric(10,2), %s, %s)"
...
def main():
    ...
    try:
        ...
        print(f"Загружено в raw.bookings: {len(rows)} строк, _loaded_at={load_ts.isoformat()}")
    finally:
        source_conn.close()
        dwh_conn.close()
